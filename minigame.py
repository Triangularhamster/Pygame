...
# 압축 해제한 폴더의 위치, 본인 폴더 위치 지정하기
folder = 'C:\\Users\\HYUKJUN\\Desktop\\mini_game'
...
# 게임 배경화면
background = pygame.image.load(folder+'\\background1.png')  # 배경화면 사진 로드
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        super().__init__()
        # 플레이어 사진 불러오기
        self.image = pygame.image.load(folder+'\\chick_tall.png')
...
class Enemy(pygame.sprite.Sprite):
    ...
    def __init__(self):
        super().__init__()
        # 적 사진 불러오기
        self.image = pygame.image.load(folder+'\\boom2.png')
        # 이미지 크기의 직사각형 모양 불러오기
        self.rect = self.image.get_rect()
        # rec 크기 축소(충돌판정 이미지에 맞추기 위함)
        self.rect = self.rect.inflate(-20,-20)
...
# 게임 BGM 설정
bgm = pygame.mixer.Sound(folder+'\\backgroundMusic.WAV')
bgm.play()
...
while True:
    ...
    if pygame.sprite.spritecollideany(P1, Enemies):
        ...
        # 플레이어
        GameDisplay.blit(background, (0, 0))
        image0 = pygame.image.load(folder+'\\chickbommed_down.png')
        image0.get_rect()
        GameDisplay.blit(image0, player_pos)

        # 폭탄
        image1 = pygame.image.load(folder+'\\boomm_down.png')
        image1.get_rect()
        ...
        bgm.stop()
        # 적과 충돌시 효과음 추가
        pygame.mixer.Sound(folder+'\\BOOM.WAV').play()
        time.sleep(0.5)
        # 게임오버화면 설정
        pygame.mixer.Sound(folder+'\\gameover.WAV').play()
